...
const menu = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            selector: (row) => row.coveragedetails,
        },
        {
            name: "Effect Date",
            selector: (row) => row.effectdate,
        },
        {
            name: "Expiry Date",
        ...
        },
        {
            ...
            cell: (row) => (
                <div>
                    ...
                    <button type="button" 
                        className="py-3 px-4 inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent text-red-500 hover:bg-red-100 hover:text-red-800 disabled:opacity-50 disabled:pointer-events-none dark:hover:bg-red-800/30 dark:hover:text-red-400" onClick={() => handleDelete(row.id)}>
                        Delete
                    </button>
                </div>
            )
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get("https://dulanga.azurewebsites.net/api/innobothealth/insurance/getAll");
    ...
    useEffect(() => {
        const result = data.filter((item) => {
            return (item.insurerName && item.insurerName.toLowerCase().includes(search.toLowerCase()));
        });
    ...
    const handleDelete = async (id) => {
        ...
        if (confirmDeleteAction) {
            try {
                await axios.delete("https://dulanga.azurewebsites.net/api/innobothealth/insurance/${id}");
                console.log("Item deleted successfully:", insurerName);
                const newData = data.filter((item) => item.insurerName !== insurerName);
                setData(newData);
                setLoading(true);
            ...
            console.log("Delete action canceled for item:", insurerName);
        }
    };
    ...
    return (
        <div>
            ...
            <div className="text-center">
                <h2 className="md:text-2xl text-xl font-bold md:leading-snug leading-snug mt-2">Insurance List</h2>
            </div>
